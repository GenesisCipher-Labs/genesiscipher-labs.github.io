...
- HomeSafe's personal learning profile stays on your device. It keeps interpretable aggregates such as walking pace, travel rhythm, coarse place familiarity and check-in responsiveness, plus at most the latest 24 dimensionless ETA differences per travel mode and time band. Those timing values carry no route, endpoint, timestamp, or trip identifier. They can add a private planning allowance after enough comparable arrivals, but never change a route's safety score. It also keeps an on-device per-region timing average; an existing aggregate may retain India or United States as its category from an earlier release even though current market availability is United States only. That average carries no city, route, or timestamp. The profile is included in Export and Delete all.
