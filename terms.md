...
- HomeSafe's personal learning profile stays on your device. It keeps interpretable aggregates such as walking pace, travel rhythm, coarse place familiarity and check-in responsiveness, plus at most the latest 24 dimensionless ETA differences per travel mode and time band. Those timing values carry no route, endpoint, timestamp, or trip identifier. They can add a private planning allowance after enough comparable arrivals, but never change a route's safety score. It also keeps a per-region timing average (India or United States) so the app can tell where its travel-time estimates run consistently long or short; that average carries no city, route, or timestamp. The profile is included in Export and Delete all.
